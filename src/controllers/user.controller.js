...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  // const delAvatar=await User.findByIdAndDelete(
  //   req.user?._id,
  //   {
...
const UpdateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImageLocalPath) {
    throw new ApiError(400, "coverImage path notfound");
  }
  const coverImage = await uploadOnCloudinary(coverImageLocalPath);
  if (!coverImage.url) {
    throw ApiError(400, "error while uploding on cloudinary");
  }
  const user = await User.findByIdAndUpdate(
    req.user?._id,
    {
  ...
  return res.status(200).json(new ApiError(200, user, "coverImage Updated"));
});
